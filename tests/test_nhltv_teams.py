...
class TestNhlTvTeams(unittest.TestCase):
    ...
    def test_getTeamFullNameByAbbreviation(self):
        self.assertEqual(self.nhlTeams.getTeam(
            "DET").fullName, "Detroit Red Wings")

    def test_getTeamIdByAbbreviation(self):
        self.assertEqual(self.nhlTeams.getTeam(
            "DET").id, 17, "expected team id 17")
    ...
    def test_getTeamAbbreviationById(self):
        self.assertEqual(self.nhlTeams.getTeam(
            17).abbreviation, "DET", "expected team DET")
    ...
    def test_raisesLookupErrorExceptionOnInvalidTeamId(self):
        ...
        errMsg = "Missing text '" + exceptionStringContains + \
            "' but got '" + str(context.exception) + "'"
        self.assertTrue(exceptionStringContains in str(
            context.exception), errMsg)

    def test_getTeamIdName(self):
        self.assertEqual(self.nhlTeams.getTeam(
            "Detroit").id, 17, "expected team id 17")
