...
class Test_nhltv_lib_download_nhl(unittest.TestCase):
    ...
    def test02_check_for_new_game_returns_json(self, mock_requests):
        ...
        json_source = self.dl.check_for_new_game("2017-03-14", "2017-03-17")
        self.assertTrue(
            isinstance(json_source, dict), "Expected json_source to be of type dict"
        )

    @patch("nhltv_lib.download_nhl.get_setting", return_value=123)
    def test03_look_for_the_next_game_to_get(self, mock_patch):
        game, favTeamHomeAway = self.dl.look_for_the_next_game_to_get(self.json_source)
        self.assertEqual(game["teams"]["away"]["team"]["abbreviation"], "DET")
    ...
    def test04_getGameId(
        self, mock_check_for_new_game, mock_requests, mock_get_setting
    ):
        ...
        self.assertTrue(
            isinstance(contentID, str), "Expected contentID to be of type str"
        )
        self.assertTrue(
